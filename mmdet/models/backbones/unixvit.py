...
class UNIXVit(DualVit):
    ...
    def __init__(self,
        ...
        for i in range(num_stages):
            if i == 0:
                ...
                patch_embed_ir = DownSamples(embed_dims[i - 1], embed_dims[i])
            ...
            out_norm = norm_layer(embed_dims[i]) if i in self.out_indices else nn.Identity()
            cur += depths[i]
    ...
    def forward_sep(self, x, x_ir, x_weight, x_ir_weight):
        ...
        for i in range(self.sep_stage):
            score_embed = self.score_embeds[i].repeat(B, 1, 1)  # diff
            mod_embed = self.mod_embeds[i].repeat(B, 1, 1)  # diff
            ...
            score_embed = extra_token[:, :score_end_idx, :] # B, 3, 64
            ...
            if self.score_embed_nums > 1:
                score_embed = (
                    ...
                    x_ir_weight / (x_weight + x_ir_weight)).unsqueeze(1)    # diff

            if self.mod_nums > 1:
                mod_embed = (mod_embed[:, 0, :] * x_weight +
                             mod_embed[:, 1, :] * x_ir_weight /
                            #  (x_weight + x_ir_weight)).unsqueeze(0)
                             (x_weight + x_ir_weight)).unsqueeze(1)     # diff

            score_weight = self.scorenet[i](score_embed)    # B, 1, C -> 2, B, 1
            x, x_ir = x.chunk(2, dim=1)
            x = x * score_weight[0].unsqueeze(-1) # diff
            x_ir = x_ir * score_weight[1].unsqueeze(-1)   # diff
    ...
    def forward_merge(self, x, semantics, x_weight, x_ir_weight):
        ...
        for i in range(self.sep_stage, self.num_stages):
            ...
            semantics = semantics_embed(semantics)
            # diff
            x = torch.cat([score_embed, mod_embed, x, x_ir, semantics], dim=1)
            ...
            x_rgb = x_rgb * score_weight[0].unsqueeze(-1) # diff
            x_ir = x_ir * score_weight[1].unsqueeze(-1) # diff
            ...
            x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1,
                                                       2).contiguous()
            x_ir = x_ir.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            x = torch.cat([x_rgb, x_ir], dim=0) # 我用这个作为输出，不就可以用来处理后续的内容了，在encoder那里做做文章
