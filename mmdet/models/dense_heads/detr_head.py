...
class DETRHead(BaseModule):
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        test_cls_score = cls_score.clone().detach().cpu().numpy()
        test_bbox_pred = bbox_pred.clone().detach().cpu().numpy()
        if np.any(np.isnan(test_cls_score)):
            raise AssertionError
        if np.any(np.isnan(test_bbox_pred)):
            raise AssertionError
