...
class Extract_Edge(BaseModule):
    def __init__(self):
        ...
        self.conv_op =  nn.Conv2d(3, 3, kernel_size=3, padding=1, bias=False, groups=3)
        self.sobel_kernel = np.array([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]], dtype='float32') / 9
        self.sobel_kernel = self.sobel_kernel.reshape((1, 1, 3, 3))
        # 卷积输出通道，这里我设置为3
        # self.sobel_kernel = np.repeat(self.sobel_kernel, 3, axis=1)   # 分组卷积就不需要了
        # 输入图的通道，这里我设置为3
        self.sobel_kernel = np.repeat(self.sobel_kernel, 3, axis=0)
        self.conv_op.weight.data = torch.from_numpy(self.sobel_kernel).float()
        freeze(self.conv_op)
