...
class DINO_Parallel_0612(DINO):
    ...
    def pre_decoder(
        ...
        ## diff
        # gaussin nll loss
        enc_outputs_var = self.bbox_head.var_branches[self.decoder.num_layers](output_memory)
        ir_enc_outputs_var = self.bbox_head.var_branches[self.decoder.num_layers + ir_extra_idx](ir_output_memory)            
    ...
    def bayesian_nms(self, info_1, info_2, iou_thresh=0.5, top_k=900, method='iou'):
        ...
        #     iou = ious[i]       # 选择当前box所计算的iou
        #     set_id = set_ids[i] # 确定属于那个set
        #     out_mask1 = iou >= iou_thresh    # 判断是不是重合
        #     out_mask2 = set_ids != set_id   # 判断是不是不同模态
        #     match_mask = out_mask1 & out_mask2

        #     # match框集
        #     match_coords = torch.cat((coords[[i]], coords[match_mask]), dim=0)
        #     match_vars = torch.cat((vars[[i]], vars[match_mask]), dim=0)
        #     # weights = torch.cat((scores[[i]], scores[match_mask]), dim=0)
        ...
        # # if torch.any(torch.isnan(selected_coords)):
        # #     raise AssertionError
        # return torch.stack(selected_coords, dim=0)

        # # 下面这种方法容易出现nan，上面的方法速度太慢了
        # # --- 1. 构建全量匹配掩码 (total_mask) ---
        # # 这个掩码矩阵将一次性确定所有 top_k 个框各自的匹配集
        # iou_mask = ious[:top_k] >= iou_thresh   # top_k, N
        # set_id_mask = set_ids[:top_k].unsqueeze(1) != set_ids.unsqueeze(0)  # top_k, N
        # match_mask = iou_mask & set_id_mask # top_k, N
        # # d. 添加自身到匹配集：
        # # 创建一个对角线为1的矩阵，确保每个框都包含在自己的融合计算中
        # self_mask = torch.eye(top_k, len(coords), dtype=torch.bool, device=coords.device)   # top_k, N
        # # e. 最终的掩码：合并匹配掩码和自身掩码
        # total_mask = (match_mask | self_mask).float() # top_k, N

        # # --- 2. 执行并行的加权平均 ---
        # # 利用矩阵乘法 (matmul) 高效完成计算
        # # a. 计算权重，使用一个极小值 epsilon 防止除以零
        # weights = 1.0 / (vars + 1e-7)    # N, 1
        # # if torch.any(torch.isnan(weights)):
        # #     raise AssertionError
        # # b. 计算加权坐标
        # weighted_coords = coords * weights  # N, 4
        # # f. 计算最终的加权平均坐标
        # selected_coords = torch.matmul(total_mask, weighted_coords) / \
        #     (torch.matmul(total_mask, weights) + 1e-7)
        # # if torch.any(torch.isnan(selected_coords)):
        # #     raise AssertionError
        # return selected_coords

        """
        一个对输入、内部计算和输出都具有极高鲁棒性的WBF实现。
        """
        # ------------------ 1. 输入净化 (防御性编程) ------------------
        # 捕获上游可能产生的任何NaN或inf，防止其干扰内部计算
        coords = torch.nan_to_num(coords, nan=0.0, posinf=1e5, neginf=-1e5)
        vars = torch.nan_to_num(vars, nan=1.0, posinf=1.0) # 让异常的vars权重变小

        # ------------------ 2. 构建匹配掩码 (无变化) ------------------
        iou_mask = ious >= iou_thresh
        set_id_mask = set_ids.unsqueeze(1) != set_ids.unsqueeze(0)
        match_mask = iou_mask & set_id_mask
        self_mask = torch.eye(len(coords), dtype=torch.bool, device=coords.device)
        total_mask = (match_mask | self_mask).float()

        # ------------------ 3. 稳定权重计算 (无变化) ------------------
        # 使用 no_grad + clamp 保证前向稳定且反向梯度正确
        vars_for_calc = vars.clone()
        with torch.no_grad():
            vars_for_calc.clamp_(min=1e-4)

        weights = 1.0 / vars_for_calc
        ...
        safe_denominator = torch.clamp(denominator, min=1e-6) # 防止梯度爆炸

        selected_coords = numerator / safe_denominator
        
        # ------------------ 5. 输出净化 (关键的最后一步) ------------------
        # 在返回之前，将函数内部可能产生的任何inf或NaN都替换为安全值。
        # 这是防止污染下游IoU计算的最后一道防线。
        safe_selected_coords = torch.nan_to_num(
            selected_coords, nan=0.0, posinf=1e5, neginf=-1e5)

        # 如果需要，可以切断梯度用于纯测试
        # return safe_selected_coords[:top_k].detach()
        
        return safe_selected_coords[:top_k]
        ...
        # match框集
        match_coords = coords.unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1)    # N, N+1, 4
        match_vars = vars.unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1)    # N, N+1, 1
        # weights = scores.unsqueeze(dim=1).unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1) * match_mask
        weights = match_mask / match_vars   # N, N+1, 1
        weighted_sum = (match_coords * weights).sum(dim=1)  # N, N+1, 4 -> # N, 4
        selected_coords = weighted_sum / weights.sum(dim=1)
        return selected_coords[top_k:]
        

    def bayesian_nms_with_score(self, info_1, info_2, iou_thresh=0.5, top_k=900, method='iou'):
        def xyxy2xyw2h2(bbox):
            _bbox = bbox_xyxy_to_cxcywh(bbox)
            _bbox[:,2] = _bbox[:,2] / 2
            _bbox[:,3] = _bbox[:,3] / 2
        ...
        def bayesian_fusion(match_score_vec):
            log_positive_scores = torch.log(match_score_vec)
            log_negative_scores = torch.log(1 - match_score_vec)
            fused_positive = torch.exp(torch.sum(log_positive_scores), dim=-1)
            fused_negative = torch.exp(torch.sum(log_negative_scores), dim=-1)
        ...
        classes = torch.cat((info_1['class'], info_2['class']), dim=0)
        # Cls Logits
        cls_logits = torch.cat((info_1['cls_logits'], info_2['cls_logits']), dim=0)
        # Probs 
        probs = torch.cat((info_1['probs'], info_2['probs']), dim=0)
        # Variances
        vars = torch.cat((info_1['vars'], info_2['vars']), dim=0)
        ...
        selected_coords = torch.zeros_like(coords)
        for i in range(coords.shape[0]):
            ...
            weighted_sum = (match_coords * weights).sum(dim=0)
            keep_coords = weighted_sum / weights.sum(dim=0)
            selected_coords[i] = keep_coords

            # match score
            match_scores = torch.cat((scores[[i]], scores[match_mask]), dim=0)        
            output_scores[i] = bayesian_fusion(match_scores)

        indexes = torch.topk(output_scores, top_k, dim=0)[1]
        return torch.gather(selected_coords, 0, indexes.unsqueeze(-1).repeat(1, 4))

        out_mask1 = ious >= iou_thresh
        out_mask2 = set_ids.unsqueeze(-1) != set_ids.unsqueeze(0)
        # match_mask = (out_mask1 & out_mask2).to(coords).unsqueeze(-1)   # N, N, 1
        match_mask = torch.cat(
            (torch.ones_like(vars), (out_mask1 & out_mask2).to(coords)), 
            dim=1).unsqueeze(-1)    # N, N+1, 1        

        # match框集
        match_coords = coords.unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1)
        match_vars = vars.unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1)
        # weights = scores.unsqueeze(dim=1).unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1) * match_mask
        weights = match_mask / match_vars
        weighted_sum = (match_coords * weights).sum(dim=1)
        selected_coords = weighted_sum / weights.sum(dim=1)

        # match score
        match_scores = scores.unsqueeze(dim=1).repeat(1, coords.shape[0] + 1, 1) * match_mask.squeeze(dim=-1)
        output_scores = bayesian_fusion(match_scores)
        indexes = torch.topk(output_scores, top_k, dim=0)[1]
        return torch.gather(selected_coords, 0, indexes.unsqueeze(-1).repeat(1, 4))
