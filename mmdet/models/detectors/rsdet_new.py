...
class RSDet_New(FasterRCNN):
    ...
    def __init__(self,
        ...
        if neck is not None: self.neck_lwir = MODELS.build(neck)
    ...
    def redundant_spectrum_removal(self, img_vis: Tensor, img_lwir: Tensor) -> Tuple[Tensor]:
        """Extract features.
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if self.Gmask:
            img_vis_unique, img_lwir_unique = self.redundant_spectrum_removal(img_vis, img_lwir)        
            x_vis = self.extract_feat(img_vis_unique)
            x_lwir = self.extract_feat_lwir(img_lwir_unique)    
            x_common= self.Gcommon(x_vis, x_lwir, img_vis_unique, img_lwir_unique)
        else:        
            x_vis = self.extract_feat(img_vis_unique)
            x_lwir = self.extract_feat_lwir(img_lwir_unique)    
            x_common= self.Gcommon(x_vis, x_lwir, img_vis_unique, img_lwir_unique)
