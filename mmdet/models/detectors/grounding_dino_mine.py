...
from ..layers.transformer.grounding_dino_layers_mine import (
    GroundingDinoTransformerDecoder_Mine, GroundingDinoTransformerEncoder_Mine)
...
class GroundingDINO_Mine(DINO):
    ...
    def __init__(self,
        ...
        if bbox_head is not None:
            ...
            bbox_head['num_pred_layer'] += 1  # for supervise instance of ir image
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = GroundingDinoTransformerEncoder_Mine(**self.encoder)
        self.decoder = GroundingDinoTransformerDecoder_Mine(**self.decoder)
    ...
    def forward_encoder(self, img : Dict, ir_img: Dict) -> Dict:
        ...
        rgb_memory, ir_memory = self.encoder(
            # rgb
            query=img['feat'],
            ...
            ir_key_padding_mask=ir_img['feat_mask'],  # for self_attn
        ...
        encoder_outputs_dict = dict(
            ...
            spatial_shapes=img['spatial_shapes'],
            ir_memory=ir_memory,
            ir_memory_mask=ir_img['feat_mask'],
            ir_spatial_shapes=ir_img['spatial_shapes'],
            )
    ...
    def pre_decoder(
        ...
        ir_enc_outputs_class = self.bbox_head.cls_branches[
            self.decoder.num_layers + 1](ir_output_memory)
        ir_enc_outputs_coord_unact = self.bbox_head.reg_branches[
            self.decoder.num_layers + 1](ir_output_memory) + ir_output_proposals
        ...
        # # for ir
        # ir_query = self.ir_query_embedding.weight[:, None, :]
        # ir_query = ir_query.repeat(1, bs, 1).transpose(0, 1)
        # fusion
        # query = self.fusion_query(query, ir_query)
        # topk_coords_unact = self.fusion_coords(topk_coords_unact, ir_topk_coords_unact)
        topk_coords_unact = topk_coords_unact + ir_topk_coords_unact
        ...
        head_inputs_dict = dict(
            ...
            ir_enc_outputs_class=ir_topk_score,
            ir_enc_outputs_coord=ir_topk_coords,
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        losses = self.bbox_head.loss(
            **head_inputs_dict, batch_data_samples=batch_data_samples,
            batch_extra_data_samples=batch_extra_data_samples)
        return losses
