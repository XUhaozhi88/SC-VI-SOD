...
class DINO_Parallel_0514(DINO):
    ...
    def batched_nms(self, info_1, info_2, iou_thresh=0.5, top_k=900):
        ...
        while len(boxes_for_nms) > 0:
            ...
            # 计算当前框与其他框的IOU
            ious = torchvision.ops.box_iou(current_box.unsqueeze(0), boxes_for_nms[1:])
    ...
    def modal_aware_nms(self, info_1, info_2, iou_thresh=0.5, top_k=900, save_path=None):
        # Raw Coords
        coords = torch.cat((info_1['coords'], info_2['coords']), dim=0)
        # Boxes
        boxes = torch.cat((info_1['bboxes'], info_2['bboxes']), dim=0)
        ...
        if save_path is not None:
            # save rgb
            rgb_filename = os.path.join(save_path, 'rgb.npy')
            rgb_save_info = torch.cat((info_1['class'].unsqueeze(1).detach(), info_1['bboxes'].detach()), dim=1)
            rgb_save_info = rgb_save_info.cpu().numpy()
            np.save(rgb_filename, rgb_save_info)
            # save ir
            ir_filename = os.path.join(save_path, 'ir.npy')
            ir_save_info = torch.cat((info_2['class'].unsqueeze(1).detach(), info_2['bboxes'].detach()), dim=1)
            ir_save_info = ir_save_info.cpu().numpy()
            np.save(ir_filename, ir_save_info)
            # save fusion
            fusion_filename = os.path.join(save_path, 'fusion.npy')
            fusion_save_info = []
        # **********************************************
        ...
        while len(boxes_for_nms) > 0:
            ...
            if save_path is not None:
                ...
                fusion_save_info.append(torch.cat((current_class.unsqueeze(0).detach(), current_box.detach()), dim=0))
                classes = classes[1:][keep_mask]
            # **********************************************
        ...
        # **********************************************
        if save_path is not None:
            fusion_save_info = torch.stack(fusion_save_info, dim=0).cpu().numpy()
            np.save(fusion_filename, fusion_save_info)
        # **********************************************
        return selected_coords

    def bayesian_nms(self, info_1, info_2, iou_thresh=0.5, top_k=900):
        # Raw Coords
        coords = torch.cat((info_1['coords'], info_2['coords']), dim=0)
        # Boxes
        boxes = torch.cat((info_1['bboxes'], info_2['bboxes']), dim=0)
        ...
        while len(boxes_for_nms) > 0:
            ...
            match_vars = 1. / match_vars
            keep_coords = match_coords * match_vars
            keep_coords = keep_coords / match_vars.sum(dim=0, keepdim=True)
            selected_coords.append(keep_coords.sum(dim=0))
    ...
    def prob_fusion(self, 
        ...
        for i in range(bs):
            ...
            if self.training:
                ...
                save_path = f'/workspace/mmdetection/results/visual/modal_aware_nms/bbox/{image_id}'
            ...
            out_coords.append(
                ...
                # 模态间nms
                self.modal_aware_nms(det, ir_det, iou_thresh=0.5, top_k=900, save_path=save_path)
                # probEn
                # self.bayesian_nms(det, ir_det, iou_thresh=0.5, top_k=900)
            )
