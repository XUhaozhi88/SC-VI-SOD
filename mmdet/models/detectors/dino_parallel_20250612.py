...
class DINO_Parallel_0612(DINO):
    ...
    def _init_layers(self) -> None:
        ...
        # #######################################################################
        # self.decoder.pop('extra_return')
        # self.decoder.pop('mode')
        # self.decoder.layer_cfg['cross_attn_cfg']['num_levels'] *= len(self.mod_list)
    ...
    def bayesian_nms(self, info_1, info_2, iou_thresh=0.5, top_k=900, method='iou'):
        ...
        return torch.topk(selected_coords, topk, dim=0)[0]
    ...
    def forward_decoder(self,                        
        ...
        inter_states, references = self.decoder(            
            ...
            **kwargs)
        #########################################################
        # num_repeat = len(self.mod_list)
        # inter_states, references = self.decoder(
        #     query=query,
        #     value=torch.cat((memory, ir_memory), dim=1),
        #     key_padding_mask=memory_mask.repeat(1, num_repeat) if memory_mask is not None else memory_mask,
        #     self_attn_mask=dn_mask,
        #     reference_points=reference_points,
        #     spatial_shapes=spatial_shapes.repeat(num_repeat, 1),
        #     # level_start_index=torch.cat((level_start_index, level_start_index + memory.shape[1]), dim=0),
        #     level_start_index=torch.cat(
        #         ([level_start_index + i * memory.shape[1] for i in range(num_repeat)]), dim=0),
        #     valid_ratios=valid_ratios.repeat(1, num_repeat, 1),
        #     reg_branches=self.bbox_head.reg_branches,
        #     **kwargs)
