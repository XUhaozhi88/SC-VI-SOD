...
class BiMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        query_states = self.v_proj(vision) * self.scale # (bsz, tgt_len, cv) -> (bsz, tgt_len, embed_dim)
        key_states = self._shape(self.l_proj(lang), -1, bsz)    # (bsz, nseq, cl) -> (bsz, nseq, embed_dim) -> (bsz, n_head, nseq, head_dim)
        value_v_states = self._shape(self.values_v_proj(vision), -1, bsz)   # (bsz, tgt_len, cl) -> (bsz, tgt_len, embed_dim) -> (bsz, n_head, tgt_len, head_dim)
        value_l_states = self._shape(self.values_l_proj(lang), -1, bsz) # (bsz, nseq, cl) -> (bsz, nseq, embed_dim) -> (bsz, n_head, nseq, head_dim)

        proj_shape = (bsz * self.num_heads, -1, self.head_dim)
        query_states = self._shape(query_states, tgt_len,
                                   bsz).view(*proj_shape)   # (bsz, tgt_len, embed_dim) -> (bsz, n_head, tgt_len, head_dim) -> (bsz * n_head, tgt_len, head_dim)
        key_states = key_states.view(*proj_shape)   # (bsz, n_head, nseq, head_dim) -> (bsz * n_head, nseq, head_dim)
        value_v_states = value_v_states.view(*proj_shape)   # (bsz, n_head, tgt_len, head_dim) -> (bsz * n_head, tgt_len, head_dim)
        value_l_states = value_l_states.view(*proj_shape)   # (bsz, n_head, nseq, head_dim) -> (bsz * n_head, nseq, head_dim)
        ...
        attn_weights_T = attn_weights.transpose(1, 2)   # (bsz * n_head, tgt_len, nseq) -> (bsz * n_head, nseq, tgt_len)
        attn_weights_l = (
            attn_weights_T -
            torch.max(attn_weights_T, dim=-1, keepdim=True)[0]) # (bsz * n_head, nseq, tgt_len)
        if self.clamp_min_for_underflow:
            # Do not increase -50000, data type half has quite limited range
        ...
        attn_weights_v = nn.functional.softmax(attn_weights, dim=-1)    # (bsz * n_head, tgt_len, nseq)

        attn_probs_v = F.dropout(
            attn_weights_v, p=self.dropout, training=self.training) # (bsz * n_head, tgt_len, nseq)
        attn_probs_l = F.dropout(
            attn_weights_l, p=self.dropout, training=self.training) # (bsz * n_head, nseq, tgt_len)

        attn_output_v = torch.bmm(attn_probs_v, value_l_states) # (bsz * n_head, tgt_len, head_dim)
        attn_output_l = torch.bmm(attn_probs_l, value_v_states) # (bsz * n_head, nseq, head_dim)
        ...
        attn_output_v = attn_output_v.transpose(1, 2)   # (bsz, n_head, tgt_len, head_dim) -> (bsz, tgt_len, n_head, head_dim)
        attn_output_v = attn_output_v.reshape(bsz, tgt_len, self.embed_dim) # (bsz, tgt_len, n_head, head_dim) -> (bsz, tgt_len, embed_dim)
