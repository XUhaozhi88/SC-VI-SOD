...
class GroundingDinoTransformerDecoderLayer_Mine(
        DeformableDetrTransformerDecoderLayer):

    def __init__(self,
                 cross_attn_ir_cfg: OptConfigType = dict(
                     embed_dims=256,
                     num_heads=8,
                     dropout=0.0,
                     batch_first=True),
        ...
        self.cross_attn_ir_cfg = cross_attn_ir_cfg
        if 'batch_first' not in self.cross_attn_ir_cfg:
            self.cross_attn_ir_cfg['batch_first'] = True
        super().__init__(**kwargs)

    def _init_layers(self) -> None:
        """Initialize self_attn, cross-attn, ffn, and norms."""
        self.self_attn = MultiheadAttention(**self.self_attn_cfg)
        self.cross_attn_ir = MultiheadAttention(**self.cross_attn_ir_cfg)
    ...
    def forward(self,
                ...
                ir_memory: Tensor = None,
                ir_attention_mask: Tensor = None,
        ...
        # cross attention between query and ir image
        query = self.cross_attn_ir(
            query=query,
            query_pos=query_pos,
            key=ir_memory,
            value=ir_memory,
            key_padding_mask=ir_attention_mask)
        query = self.norms[1](query)
        # cross attention between query and rgb image
...
class GroundingDinoTransformerEncoder_Mine(DeformableDetrTransformerEncoder):

    def __init__(self, ir_layer_cfg: ConfigType,
    ...
    def _init_layers(self) -> None:
        ...
        self.ir_layers = ModuleList([
            DeformableDetrTransformerEncoderLayer(**self.ir_layer_cfg)
            for _ in range(self.num_layers)
        ])
        ...
        # self.fusion_layers = ModuleList([
        #     checkpoint_wrapper(BiAttentionBlock_Mine(**self.fusion_layer_cfg))
        #     for _ in range(self.num_layers)
        # ])        
        ...
        if self.num_cp > 0:
            ...
            # self.ir_layers = checkpoint_wrapper(self.ir_layers)
            # self.fusion_layers = checkpoint_wrapper(self.fusion_layers)
            for i in range(self.num_cp):
                self.layers[i] = checkpoint_wrapper(self.layers[i])
                self.ir_layers[i] = checkpoint_wrapper(self.ir_layers[i])
    ...
    def forward(self,
        ...
        for layer_id, layer in enumerate(self.layers):
            if self.fusion_layers:
                output, ir_output = self.fusion_layers[layer_id](
                ...
                # output, ir_output = self.fusion_layers[layer_id](
                #     features=output,
                #     ir_features=ir_output,
                #     level_start_index=level_start_index,
                #     ir_level_start_index=ir_level_start_index,
                #     attention_mask=key_padding_mask,
                #     ir_attention_mask=ir_key_padding_mask
                # )
            if self.ir_layers:   
            ...
            output = layer(
                ...
                level_start_index=level_start_index,
                key_padding_mask=key_padding_mask)
        return output, ir_output


class GroundingDinoTransformerDecoder_Mine(DinoTransformerDecoder):
    def __init__(self, *args, num_cp, **kwargs) -> None:
        self.num_cp = num_cp
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = ModuleList([
            GroundingDinoTransformerDecoderLayer_Mine(**self.layer_cfg)
            for _ in range(self.num_layers)
        ])
        self.embed_dims = self.layers[0].embed_dims
