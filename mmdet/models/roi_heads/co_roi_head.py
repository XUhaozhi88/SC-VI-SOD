...
class CoStandardRoIHead(StandardRoIHead):
    ...
    def loss(self, x: Tuple[Tensor], rpn_results_list: InstanceList,
        ...
        max_proposal = 2000 # 感觉这个值没有意义啊，sampler设置的是512，一定是小于2000这个数的
        ...
        if self.with_bbox:
            ...
            # 下面一段代码的作用就是根据roi head的检测结果生成了新的proposal
            bbox_targets = bbox_results['bbox_targets'] # labels, label_weights, bbox_targets, bbox_weights
            for res in sampling_results:
                max_proposal = min(max_proposal, res.bboxes.shape[0])
            ori_coords = bbox2roi([res.bboxes for res in sampling_results]) # B*max_proposal, 5(img_id+coords)
            ori_proposals, ori_labels, \
                ori_bbox_targets, ori_bbox_feats = [], [], [], []
            for i in range(num_imgs):
                idx = (ori_coords[:, 0] == i).nonzero().squeeze(1)
                idx = idx[:max_proposal]    # 这一行的实现很奇怪，每个图片对应的ori_coords就是max_proposal
                ori_proposal = ori_coords[idx][:, 1:].unsqueeze(0)
                ori_label = bbox_targets[0][idx].unsqueeze(0)   # labels
                ori_bbox_target = bbox_targets[2][idx].unsqueeze(0)
                ori_bbox_feat = bbox_results['bbox_feats'].mean(-1).mean(-1)    # roi feat(N, C, 7x7 -> N, C)
