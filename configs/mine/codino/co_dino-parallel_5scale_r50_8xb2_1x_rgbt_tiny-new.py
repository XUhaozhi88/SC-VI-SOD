...
model = dict(
    ...
    rpn_head=dict(
        ...
        anchor_generator=dict(
            ...
            ratios=[0.5, 1.0, 2.0],
            strides=[4, 8, 16, 32, 64, 128] if num_levels == 5 else [4, 8, 16, 32, 64]),   # Different from the Faster RCNN, 这个要和encoder的输出层数一样（是encoder的输出reshape HxW之后作为特征图输入）
        bbox_coder=dict(
    ...
    roi_head=[
        dict(
            ...
            bbox_roi_extractor=dict(
                ...
                out_channels=256,
                featmap_strides=[4, 8, 16, 32, 64] if num_levels == 5 else [4, 8, 16, 32], # Different from the Faster RCNN , 这个要和encoder的输出层数一样
    ...
    OneStage_head=[
        dict(
            ...
            anchor_generator=dict(
                ...
                octave_base_scale=8,
                scales_per_octave=1,
                strides=[4, 8, 16, 32, 64, 128] if num_levels == 5 else [4, 8, 16, 32, 64]),   # Different from the ATSS, 这个要和encoder的输出层数一样
            bbox_coder=dict(
                type='DeltaXYWHBBoxCoder',
                target_means=[.0, .0, .0, .0],
                target_stds=[0.1, 0.1, 0.2, 0.2]),
