...
model = dict(
    ...
    bbox_head=dict(
        ...
        # loss_var=dict(type='GaussianNLLLoss', loss_weight=1.0)    # 1.0有点大，导致训练不稳定
        loss_var=dict(type='GaussianNLLLoss', loss_weight=0.1)    # eps变大控制负值大小，loss_weight变小减小影响
        # loss_var=dict(type='GaussianNLLLoss', eps=1e-4, loss_weight=0.1)    # eps变大控制负值大小，loss_weight变小减小影响
        # loss_var=dict(type='GaussianNLLLoss', eps=1e-3, loss_weight=0.05)    # eps变大控制负值大小，loss_weight变小减小影响
        # loss_var=dict(type='GaussianNLLLoss', eps=1e-6, loss_weight=0.0)    # eps变大控制负值大小，loss_weight变小减小影响
        ),
