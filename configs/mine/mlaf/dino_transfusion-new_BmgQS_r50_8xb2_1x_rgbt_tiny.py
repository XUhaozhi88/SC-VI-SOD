...
num_cross_atten=2
# extra_return=[]
extra_return=['rgb', 'ir']  # this is for encoder output and head input
model = dict(
    type='DINO_TransFusion',
    num_queries=900,
    extra_num_queries=100,  # this is for encoder output and head input
    ...
    encoder=dict(
        ...
        layer_cfg=dict(
            # receptive_field_sizes=[9, 7, 5, 3],   # deformable attention
            receptive_field_sizes=[3, 3, 3, 3], # simple attention
            num_cross_atten=num_cross_atten,
            self_attn_cfg=dict(embed_dims=256, num_heads=8, num_levels=4, dropout=0.0), # deformable attention
            # rgb_self_attn_cfg=dict(embed_dims=256, num_levels=4, dropout=0.0),        # deformable attention
            # ir_self_attn_cfg=dict(embed_dims=256, num_levels=4, dropout=0.0),         # deformable attention
            cross_attn_cfg=dict(embed_dims=256, num_heads=8, dropout=0.0),            # simple attention
            # cross_attn_cfg=dict(embed_dims=256, num_heads=8, num_levels=4, dropout=0.0),    # deformable attention
...
test_pipeline = [
    ...
    dict(type='MultiModalResize', 
         scale=(800, 1333), keep_ratio=True, mod_list=_base_.mod_list, 
